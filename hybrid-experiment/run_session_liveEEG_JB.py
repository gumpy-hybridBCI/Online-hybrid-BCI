...
# 27.03.2018
# modification of run_session_liveEEG.py to allow live processing and classification of EEG with a CNN
# run_session_liveEEG.py  by:  Mirjam Hemberger, mirjam.hemberger@tum.de
...
class MyApp_liveEEG_JB():
    def __init__(self, trial_count, trials_notlive, Fs=256, age=25, gender='male', with_feedback=False):
        ...
        self.record_data_liveEEG = RecordData_liveEEG_JB(Fs, age, gender, with_feedback)
    ...
    def classify_notlive(self):
        if __name__ == '__main__':
            ...
            self.liveEEG = liveEEG(self.datapath, self.filename_notlive)
            self.liveEEG.fit()
    ...
    def run_trial(self, with_feedback=False):
        if self.state == 1:
            ...
            # state 2: do classification of notlive data
            ...
            # state 3: do live classification
            # first classify the data from previous trial and then start new trial
            filename_live, datapath = self.record_data_liveEEG.stop_recording_and_dump_live() 
            self.current_classifier, pred_true = self.liveEEG.classify_live(self.record_data_liveEEG.get_last_trial(filename_live))
...
if __name__ == "__main__":
    ...
    app = MyApp_liveEEG_JB(args["trials"], args["trials_notlive"], args["Fs"], args["age"], gender=args["gender"], with_feedback=args["with_feedback"])
